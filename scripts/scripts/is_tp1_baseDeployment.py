...
class DataCollection(threading.Thread):
    def __init__(self):
        ...
        writeDB(createRateTable)      
        
        if(readDB("SELECT COUNT(*) AS RowCnt FROM rate")[0][0] == 0):
            writeDB("""INSERT INTO rate (rate) VALUES (1)""")
    ...
    def run(self):
        while True:
            ...
            if x==None:
                ...
                writeDB("""INSERT INTO accel (x,y,z) VALUES (%s,%s,%s)""", (x,y,z))              
                
            time.sleep(readDB('SELECT * FROM rate order by id desc limit 1')[0][1])
        return None
...
class UpdateRateAPI(Resource):
    ...
    def get(self):
        a=readDB("""SELECT * FROM rate order by id desc limit 1""")[0][1]
        print(a + "............")
        #return a
    ...
    def put(self):    
        ...
        writeDB("""INSERT INTO rate (rate) VALUES (%s)""", str(3))
        #return readDB('SELECT * FROM rate order by id desc limit 1')[0][1]
        return cache.get('rate')
